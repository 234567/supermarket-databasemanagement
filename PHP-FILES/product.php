...
h1{  background-color:#F37D2E;
	...
	font-size-20;
		...
		table,th{
		       border:1px solid rgba(252, 29, 24, 0.87);}
		</style>
		</head>
<body>
<h1>
...
while($row = mysqli_fetch_object($result))
{
 ...
 $db->query("UPDATE inventory SET  iquantity = iquantity-{$y} WHERE ipid = '{$x}'  ");
}
?>
...
<?php
$customerphone = $_POST["cid"];

$results = $db->query("SELECT cid FROM customer WHERE ccontact_no= '{$customerphone}'");
...
$db->query("INSERT INTO transaction(tcid,tpid,tpname,tquantity,tmrp,tdiscount,tnet_amount,tcontact_no) VALUES
 ({$cid},'{$a[$i]}','{$c[$i]}',{$b[$i]},{$d[$i]},{$e[$i]},{$f[$i]},'{$customerphone}')");
